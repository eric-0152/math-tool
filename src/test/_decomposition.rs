...
#[test]
fn lu() {
    let mut mat = Matrix::random_matrix(5, 3, -100.0, 100.0, true).round(0);
    let (l, u, p) = mat.lu();
    assert_eq!(
        (&(&l * &u) - &(&p * &mat)).round(8).entries,
        Matrix::zeros(5, 3).entries
    );
