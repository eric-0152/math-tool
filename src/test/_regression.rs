use math_tool::{regression, vector::Vector};
...
fn polynomial() {
    ...
    let y_hat = regression::polynomial_regression(&x, &y, DEGREE)
        .unwrap()
        .round(ROUND);
...
fn exponential() {
    ...
    let y_hat = regression::exponential_regression(&x, &y)
        .unwrap()
        .round(ROUND);
    assert_eq!(y_hat.entries, params.entries);
}
...
fn gaussian() {
    ...
    let y_hat = regression::gaussian_1d_regression(&x, &y)
        .unwrap()
        .round(ROUND);
    assert_eq!(y_hat.entries, params.entries);
}
